...
internal static class Av1InterPredictor
{
        ...
        int subsamplingY)
    {
        ...
        int dx = (bx4 * horizontalMultiplier) + (motionVector.X >> (3 + subsamplingX));
        int dy = (by4 * verticalMultiplier) + (motionVector.Y >> (3 + subsamplingY));
        int mx = (motionVector.X & (15 >> horizontalShift)) << horizontalShift;
        int my = (motionVector.Y & (15 >> verticalShift)) << verticalShift;

        int width = blockWidth4 * horizontalMultiplier;
        int height = blockHeight4 * verticalMultiplier;
        int filterType = filter1 | (filter0 << 2);
